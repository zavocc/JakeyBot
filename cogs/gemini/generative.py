...
class ChatControlsView(discord.ui.View):
    def __init__(self, prompt: str, chat: str, path: str, model: str, is_ephemeral: bool, prompt_count: int = 0):
        super().__init__()
        self.prompt = prompt
        self.chat = chat
        self.path = path
    ...
    @discord.ui.button(label="Export as markdown", style=discord.ButtonStyle.green, emoji="📜", row=0)
    async def button_callback(self, button, interaction):
        # Send the file
        async with aiofiles.open(self.path, "w+") as w:
            await w.write(f"# Prompt: {self.prompt}\n")
            await w.write(self.chat.text)

        await interaction.response.send_message("Ok, I saved your response into markdown file!", file=discord.File(self.path, "response.md"), ephemeral=True)
        await aiofiles.os.remove(self.path)

    @discord.ui.button(label="Show chat information", style=discord.ButtonStyle.gray, emoji="ℹ️", row=1)
    async def se_button_callback(self, button, interaction):
        if self.is_ephemeral:
            await interaction.response.send_message(inspect.cleandoc(f"""
                                > 📃 Context size: **{self.prompt_count}** of {environ.get("MAX_CONTEXT_HISTORY", 20)}
                                > ✨ Model used: **{self.model}**
                                > 🅰️ Chat token count: **{self.chat.usage_metadata.total_token_count}**
                                """), ephemeral=True)
        else:
             await interaction.response.send(f"> 📃 Responses isn't be saved\n> ✨ Model used: **{self.model}**\n> 🅰️ Chat token count: **{self.chat.usage_metadata.total_token_count}**",
                                             ephemeral=True)
...
class BaseChat(commands.Cog):
    ...
    async def ask(self, ctx, prompt: str, attachment: discord.Attachment, model: str,
        ...
        # Temp file
        _tempfile = f"{environ.get('TEMP_DIR')}/response{random.randint(6000,7000)}.md"

        # Embed the response if the response is more than 2000 characters
        # Check to see if this message is more than 2000 characters which embeds will be used for displaying the message
        if len(answer.text) > 4096:
            # Send the response as file
            async with aiofiles.open(_tempfile, "w+") as f:
                await f.write(answer.text)
            await ctx.respond("⚠️ Response is too long. But, I saved your response into a markdown file", file=discord.File(_tempfile, "response.md"))
            await aiofiles.os.remove(_tempfile)
            ...
            await ctx.respond(embed=embed, view=ChatControlsView(prompt, answer, _tempfile, model, append_history, _prompt_count))
        else:
            await ctx.respond(answer.text, view=ChatControlsView(prompt, answer, _tempfile, model, append_history, _prompt_count))
