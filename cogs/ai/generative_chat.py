...
class BaseChat():
    ...
    async def _ask(self, prompt: Message):
        # Set default model
        _model_props = await fetch_model(model_alias=(await self.DBConn.get_key(guild_id=prompt.author.id, key="default_model")) or "gemini::gemini-2.5-flash-nonthinking")
