...
class BaseChat(commands.Cog):
    ...
    async def ask(self, ctx, prompt: str, attachment: discord.Attachment, model: str,
        ...
        _system_embed.add_field(name="Context size", value=f"{_result["prompt_count"]} of {environ.get('MAX_CONTEXT_HISTORY', 20)}")
        ...
        if len(_result["answer"]) > 4096:
            ...
            _embed.set_author(name=self.author)
            _embed.add_field(name="Model used", value=_model_name)
            _embed.add_field(name="Context size", value=f"{len(_result["chat_thread"])} of {environ.get('MAX_CONTEXT_HISTORY', 20)}")
