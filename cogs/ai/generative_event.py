...
class BaseChat():
    ...
    async def _ask(self, prompt: Message):
        ...
        # For messages more than 2000 and less than 4096 characters, it will be sent as embed
        if len(_formatted_response) > 2000 and len(_formatted_response) < 4096:
            _system_embed = discord.Embed(
                ...
                description=_formatted_response,
                color=discord.Color.random()
            )
            _system_embed.set_footer(text="Responses generated by AI may not give accurate results! Double check with facts!")

            # Add model used and its provider
            _system_embed.add_field(name="Model used", value=_model_name)
            _system_embed.add_field(name="Model provider", value=_model_provider)

            # Check if chat is ephemeral
            if not _append_history:
                _system_embed.add_field(name="Privacy", value="Ephemeral chat")
